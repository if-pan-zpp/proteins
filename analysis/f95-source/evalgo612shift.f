      subroutine evalgo612shift(nres,x0,y0,z0,fx,fy,fz,b,icmap,eij,
     ...
     $           jstep,cutparam,nr_l)
      ...
      real*8 sij(kcont),dij(kcont)
      real*8 eij(nres,nres) 
      integer krist(nres*nres,2)
      integer ichain(nch,2)
      real*8 epot,H1,H2,rcut,rcut2,sigma0,cutparam,s612
      integer i,j,k,i1,i2,inc,jstep,nr_l
      real*8 dx,dy,dz,dxn,dyn,dzn,r,r2,rb,rn,rb2,s
      real*8 ene,fce,repx,repy,repz
      real*8 dis,d6,d3,d2,eps,dcut
      ...
      do k=1,kcont
       ...
       if (icmap(i,j).eq.1) then
        ...
        d3=d2*dis
        d6=d3*d3
        ene=4.d0*eps*d6*(d6-1.d0)
        epot=epot+ene
        fce=24.d0*eps*d6*(1.d0-2.d0*d6)*dis/s612
        repx=-fce*dx/r
        repy=-fce*dy/r
        repz=-fce*dz/r
        fx(i)=fx(i)+repx
