      subroutine get_rem_contacts(pdbfile,nres,iseq,seq,ch,icmap,
     $           kcont,kk_map,rr,arg,outputfile,kk_out,iresn,nssb3)
...
15    read(rr,'(a)',end=20,err=20) line
      if(line(1:13).ne.'REMOVECONTACT') goto 15
     ...
     $            form='formatted',status='unknown')
      ...
      write(kk_out,'(a37)')'CONTACTS TO REMOVE ARE DEFINED INCORRECTLY!'
      close(kk_out)
      stop
      endif
      test1=0
...
20    close(rr)
      if (nssb.gt.0) then
       write(kk_map,'(/,a)') '# REMOVED CONTACTS'
       do k=1,nssb
        i1=ksb(k,1)
        i2=ksb(k,2)
