      subroutine evalgo61012(nres,x0,y0,z0,fx,fy,fz,b,icmap,eij,
     ...
     $           jstep,cutparam,nr_l)
      ...
      integer klist(kcont,2),conttime(kcont)
      real*8 dij(kcont)
      real*8 eij(nres,nres)
      integer krist(nres*nres,2)
      integer ichain(nch,2)
      real*8 epot,H1,H2,rcut,rcut2,sigma0
      integer i,j,k,i1,i2,inc,nr_l,jstep
      real*8 dx,dy,dz,dxn,dyn,dzn,r,r2,rb,rn,rb2
      real*8 ene,fce,repx,repy,repz,cutparam,dcut
      ...
      do k=1,kcont
       ...
       if (icmap(i,j).eq.1) then
        ...
        dm10=dm5*dm5
        dm12=dm6*dm6
        ene=4.d0*eps*(13.d0*dm12-18.d0*dm10+4.d0*dm6)
        epot=epot+ene
        fce=-48.d0*eps*(13.d0*dm12-15.d0*dm10+2.d0*dm6)/r2
        repx=-fce*dx
        repy=-fce*dy
        repz=-fce*dz
        fx(i)=fx(i)+repx
