      subroutine get_add_bonds(pdbfile,nres,iseq,seq,ch,icmap,
     $           kcont,kk_map,rr,arg,outputfile,kk_out,iresn,nssb1)
...
15    read(rr,'(a)',end=20,err=20) line
      ...
      do j=1,nres
       if ((ch(j).eq.chain1).and.(iresn(j).eq.cys1)) then
        !write(*,*)iresn(j), cys1,j
        ksb(nssb,1)=j
        test1=1
       endif 
       if (ch(j).eq.chain2.and.iresn(j).eq.cys2) then
        !write(*,*)iresn(j), cys2,j
        ksb(nssb,2)=j
        test2=1
       endif
      enddo
      
      !write(*,*) ksb(1,1),ksb(1,2)
...
20    close(rr)
      if (nssb.gt.0) then
       write(kk_map,'(/,a)') '# ADDITIONAL BONDS'
       
       do k=1,nssb
        i1=ksb(k,1)
        i2=ksb(k,2)
!40      format(a1,1x,i6,2x,a3,2x,i4,1x,a1,6x,a3,2x,i4,1x,a1)
40        format(a1,1x,i6,2x,i4,2x,a3,1x,a1,1x,i3, 5x, 
     $         i4,2x,a3,1x,a1,1x,i3)
        
        if(icmap(i1,i2).eq.1) then
         write(kk_map,40)'B',k,
!     $     seq(i1),iseq(i1),ch(i1),seq(i2),iseq(i2),ch(i2)
     $   i1,seq(i1),ch(i1),iresn(i1),i2,seq(i2),ch(i2),
     $     iresn(i2)
